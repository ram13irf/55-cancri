...
// GitHub Pages -repositoriopolku, pitää vastata vite.config.js:n base-arvo
const basePath = '/55-cancri/'; 
...
const textures = { 
  sun: loader.load(basePath + 'textures/8k_sun.jpg'), 
  b: loader.load(basePath + 'textures/gas.jpg'), 
  c: loader.load(basePath + 'textures/gas.jpg'), 
  d: loader.load(basePath + 'textures/gas.jpg'), 
  f: loader.load(basePath + 'textures/ice.jpg') 
}; 
...
function createLabel(text, color, size=12, height=3){ 
  ...
  ctx.font = `${size}px Arial`; // FIX: template literal
...
planetData.forEach(d=>{ 
  ...
  const glowMat = new THREE.MeshBasicMaterial({ 
    ...
    transparent: true, 
    opacity: 0.5, 
    side: THREE.BackSide // FIX: properly set back side
  }); 
